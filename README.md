...
Mobile-first static site: a **Mapbox GL JS** map centered on **Cluj-Napoca, Romania**, using **MapTiler Streets** as the basemap, plus public toilets from **OpenStreetMap** (Overpass API) with a **local fallback** if the network fails and **your own markers** stored in the browser.
...
- **MapTiler Streets** vector style via **[Mapbox GL JS](https://docs.mapbox.com/mapbox-gl-js/)**, with custom touch-friendly zoom buttons  
